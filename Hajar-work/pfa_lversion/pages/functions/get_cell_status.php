    ...
    if (isset($_SESSION['cne'])) {
    	...
    	if ($_SESSION['cne'] == $_SESSION['presidentC']['cne']) {
	       ...
	       //NOT YET JOINED CELLS
	        $sql_njoined_cell="select * from cellule cu where id_club=? and not exists(
	    	    select id_cellule, intitule, id_club, 'M' as statut
		    	from inscription join cellule using(id_cellule) where cne=? and etat_insc=? and date_i_fin is null
		        UNION
		        select id_cellule, intitule, id_club, 'R' as statut
		    	from responsable r join cellule using(id_cellule) where r.cne=? and date_r_fin is null)";
	    	$stmt_joined_cell=$pdo->prepare($sql_njoined_cell);
	        $stmt_joined_cell->execute([$_GET['i'], $_SESSION['cne'], $etat_insc, $_SESSION['cne']]);
